...
Skybox::Skybox(const unsigned int& _skyboxProgram, Model* modelForSkybox) : skyboxProgram(_skyboxProgram), skyboxModel(modelForSkybox) {
...
void Skybox::renderSkybox(const glm::mat4& projection, const glm::mat4& view, const unsigned int& texture) {
	glDepthMask(GL_FALSE); // make sure writing to update depth test is off
	glCullFace(GL_FRONT); // drawing inside of cube!
