...
Terrain::Terrain(const std::vector<glm::vec3> verts, const std::vector<glm::vec3> normals, const std::vector<glm::vec2> texCoords, std::vector<unsigned int> indices, glm::vec3 pos, const float size, GLuint tex_dif, GLuint tex_spec, const float heights[VERTEX_COUNT][VERTEX_COUNT], float tiling) : TERRAIN_SIZE(size), position(pos), tiling(tiling)/*, texture(tex_dif), specularMap(tex_spec)*/ {
...
Terrain::Terrain(const std::vector<glm::vec3> verts, const std::vector<glm::vec3> normals, const std::vector<glm::vec2> texCoords, const std::vector<glm::vec3> tangents, const std::vector<glm::vec3> bitangents, std::vector<unsigned int> indices, glm::vec3 pos, const float size, GLuint tex_dif, GLuint tex_spec, GLuint tex_norm, const float heights[VERTEX_COUNT][VERTEX_COUNT], float tiling) : TERRAIN_SIZE(size), position(pos), tiling(tiling)/*, texture(tex_dif), specularMap(tex_spec), normalMap(tex_norm)*/ {
...
//	VAO->enableVertexAttribArray_I(5, 1, b::gl_byte, 0, 0);
	...
	glm::mat4 model;
	Common::createModelMatrix(model, this->position, 0.0f, glm::vec3(1.0f, 1.0f, 1.0f));
	material = new Material(tex_dif, tex_spec, tex_norm, model, tiling);

	init(true);
}

void Terrain::init(bool ifMapped) {
...
glm::mat4 * Terrain::getModelMatrix_Grass() {
	return &grassModelMat;
}
